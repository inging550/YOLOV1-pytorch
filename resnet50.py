...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:  # 步长不等于1或者输入通道数不等于输出通道数的4倍则进行一次叠加
    ...
    def forward(self, x):  # 输入batchsize*3*448*448
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        # print(x.shape)  # 64*112*112
        x = self.layer1(x)
        # print(x.shape)  # 256*112*112
        x = self.layer2(x)
        # print(x.shape)  # 512*56*56
        x = self.layer3(x)
        # print(x.shape)  # 1024*28*28
        x = self.layer4(x)   # batch_size*2048*14*14
        # 从以上格式到yolo1需要的batch_size*7*7*30的格式有很多实现方法
        x = self.layer5(x)   # batch_size*256*14*14
        x = self.avgpool(x)  # batch_size*256*7*7
        x = self.conv_end(x) # batch_size*30*7*7
        x = self.bn_end(x)
        x = torch.sigmoid(x)
        x = x.permute(0, 2, 3, 1)  # bitch_size*7*7*30
        return x
